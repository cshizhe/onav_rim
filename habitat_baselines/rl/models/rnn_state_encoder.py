...
class RNNStateEncoder(nn.Module):
    ...
    def episode_forward(self, x, init_hidden_states):
        r'''Forward a mini-batch of episodes
        Args:
            x: (N, T, D)
            hidden_states: zero state
        '''
        outputs, hidden_states = self.rnn(x.permute(1, 0, 2), init_hidden_states)
    ...
    def seq_forward(self, x, hidden_states, masks):
        ...
        # a zero in it.
        # because multiple episodes can exist (0 is the start of a new episode)
        ...
        hidden_states = self._unpack_hidden(hidden_states)  # (nlayer, N, d)
        outputs = []
    ...
    def forward(self, x, hidden_states, masks=None, seq_single_episode=False):
        # if x.size(0) == hidden_states.size(1):
        #     return self.single_forward(x, hidden_states, masks)
        # else:
        if seq_single_episode:
            return self.episode_forward(x, hidden_states)
        else:

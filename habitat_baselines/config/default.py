...
# -----------------------------------------------------------------------------
_C.MODEL = CN()
_C.MODEL.num_ft_views = None
# -----------------------------------------------------------------------------
# MAPPING
# -----------------------------------------------------------------------------
_C.MAPPING = CN()
_C.MAPPING.map_size_cm = 2400       # cm
_C.MAPPING.map_resolution = 5       # cm
_C.MAPPING.global_downscaling = 2   # full map -> local map
_C.MAPPING.vision_range = 100       # grids (100x100) in local map updated per step
_C.MAPPING.du_scale = 1             # subsampling depth points in the image
_C.MAPPING.map_pred_threshold = 1.0
_C.MAPPING.exp_pred_threshold = 1.0
_C.MAPPING.collision_threshold = 0.2


def add_pano_sensors_to_config(config):
    ...
    # The value of the orientation as "[x,y,z]" mean that the pitch, yaw, and roll of the sensor are x, y, z, respectively 
    # And it means to look more toward the ceiling if x is positive
    # In addition, the value of x needs to be set as radian (i.e., 30/180*pi) rather than degree (30 degrees).

    num_ft_views = config.MODEL.num_ft_views

    num_cameras = 12 # horizontal rotation: 30 degrees
    if num_ft_views == 1:
        return config
    elif num_ft_views == 2:
        orient = [(0, 0, 0), (- np.pi / 6, 0, 0)]
    elif num_ft_views == '2_up':
        orient = [(0, 0, 0), (np.pi / 6, 0, 0)]
    elif num_ft_views == 3:
        orient = [(0, 0, 0), (np.pi / 6, 0, 0), (- np.pi / 6, 0, 0)]
    elif num_ft_views == 12:
        orient = [(0, np.pi * 2 / num_cameras * i, 0) for i in range(num_cameras)]
    elif num_ft_views == 14:
        orient = [(0, np.pi * 2 / num_cameras * i, 0) for i in range(num_cameras)] + \
                 [(np.pi / 6, 0, 0), (- np.pi / 6, 0, 0)]
    else:
        raise NotImplementedError(f'unspport #cameras: {num_ft_views}')
    
    config.defrost()

    # RGB_SENSOR
    config.TASK_CONFIG.SIMULATOR.RGB_SENSOR.ORIENTATION = orient[0]
    for camera_id in range(1, len(orient)):
...
def get_config(
    ...
    if config.MODEL.num_ft_views is None:
        config.MODEL.num_ft_views = 1

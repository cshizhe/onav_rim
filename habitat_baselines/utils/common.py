...
class CategoricalNet(nn.Module):
    def __init__(self, num_inputs: int, num_outputs: int, clf_net=None) -> None:
        super().__init__()
    ...
    def forward(self, x: Tensor, masks=None) -> CustomFixedCategorical:
        x = self.linear(x)
        if masks is not None:
            x = x.masked_fill(masks, -float('inf'))

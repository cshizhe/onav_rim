...
import habitat.utils.rotation_utils as ru
...
def transform_pose_t(XYZ, current_pose, device):
    ...
    Output:
        XYZ : ...x3
    """
    # R = ru.get_r_matrix([0., 0., 1.], angle=current_pose[2] - np.pi / 2.) # facing east as 0
...
def splat_feat_nd(init_grid, feat, coords):
    ...
    for d in range(n_dims):
        ...
        for ix in [0, 1]:
            ...
            # put the position with decimals into the neighboring 2 indexes
            wts_ix = 1 - torch.abs(pos - pos_ix)

            wts_ix = wts_ix * safe_ix
            pos_ix = pos_ix * safe_ix

            pos_d.append(pos_ix)    # (B, nPts), indexes in the grid dimension
            wts_d.append(wts_ix)    # (B, nPts), weights in the index of current indexes
    ...
    for ix_d in itertools.product(*l_ix):
        # ix_d: (0, 0, 0), (0, 0, 1), ..., (1, 1, 1)
        wts = torch.ones_like(wts_dim[0][0])    # (B, nPts)

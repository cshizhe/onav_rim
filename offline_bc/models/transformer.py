...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, src,
        ...
        if self.batch_first:
            ...
            if (pos is not None) and (len(pos.size()) == 3):
        ...
        for layer in self.layers:
            ...
            if return_attn_weights:
                layer_attn_weights.append(output[1])
            output = output[0]
        ...
        if return_attn_weights:
            return output, layer_attn_weights
        return output


class TransformerDecoder(nn.Module):

    def __init__(self, decoder_layer, num_layers, norm=None, return_intermediate=False, batch_first=True):
        super().__init__()
    ...
    def forward(self, tgt, memory,
        ...
        output = tgt
        if self.batch_first:
            output = output.transpose(0, 1) # (length, batch, hidden_dim)
            memory = memory.transpose(0, 1)
            # TODO: pos embedding (b, d), (b, m, d)
...
def triangular_mask(size, device, diagonal_shift=1):
    ...
    '''
    square = torch.triu(torch.ones(size, size, device=device).bool(), diagonal=diagonal_shift)
